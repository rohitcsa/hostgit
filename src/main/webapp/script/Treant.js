...
( function () {
	...
	/**
	 * ImageLoader is used for determining if all the images from the Tree are loaded. Node size (width, height) can be correctly determined only when all inner images are loaded
	 */
	...
	/**
	 * Class: TreeStore TreeStore is used for holding initialized Tree objects Its purpose is to avoid global variables and enable multiple Trees on the page.
	 */
	var TreeStore = {
	...
	Tree.prototype = {
		...
		addNode : function ( parentTreeNode, nodeDefinition ) {
			...
			this.CONFIG.callback.onAfterAddNode.apply( this, [
					oNewNode, parentTreeNode, nodeDefinition
			] );
		...
		positionTree : function ( callback ) {
			...
			if ( this.imageLoader.isNotLoading() ) {
				...
				if ( !this.loaded ) {
					...
					self.CONFIG.callback.onTreeLoaded.apply( self, [
						root
					] );
					this.loaded = true;
				}
		...
		/**
		 * In a first post-order walk, every node of the tree is assigned a preliminary x-coordinate (held in field node->prelim). In addition, internal nodes are given modifiers, which will be used to move their children to the right (held in field node->modifier).
		 * 
		 * @param {TreeNode}
		 *            node
		 * @param {number}
		...
		/*
		 * Clean up the positioning of small sibling subtrees. Subtrees of a node are formed independently and placed as close together as possible. By requiring that the subtrees be rigid at the time they are put together, we avoid the undesirable effects that can accrue from positioning nodes
		...
		/*
		 * During a second pre-order walk, each node is given a final x-coordinate by summing its preliminary x-coordinate and the modifiers of all the node's ancestors. The y-coordinate depends on the height of the tree. (The roles of x and y are reversed for RootOrientations of EAST or WEST.)
		 */
		secondWalk : function ( node, level, X, Y ) {
			if ( level <= this.CONFIG.maxDepth ) {
				var xTmp = node.prelim + X, yTmp = Y, align = this.CONFIG.nodeAlign, orinet = this.CONFIG.rootOrientation, levelHeight, nodesizeTmp;
		...
		/**
		 * position all the nodes, center the tree in center of its container 0,0 coordinate is in the upper left corner
		 * 
		 * @returns {Tree}
		 */
		positionNodes : function () {
			...
			treeWidth = treeSize.x.max - treeSize.x.min, treeHeight = treeSize.y.max - treeSize.y.min,
			...
			deltaX = containerCenter.x - treeCenter.x, deltaY = containerCenter.y - treeCenter.y,

			// all nodes must have positive X or Y coordinates, handle this with offsets
			negOffsetX = ( ( treeSize.x.min + deltaX ) <= 0 ) ? Math.abs( treeSize.x.min ) : 0, negOffsetY = ( ( treeSize.y.min + deltaY ) <= 0 ) ? Math.abs( treeSize.y.min ) : 0, i, len, node;
			...
			for ( i = 0, len = this.nodeDB.db.length; i < len; i++ ) {
				...
				self.CONFIG.callback.onBeforePositionNode.apply( self, [
						node, i, containerCenter, treeCenter
				] );
		...
		setConnectionToParent : function ( treeNode, hidePoint ) {
			var stacked = treeNode.stackParentId, connLine, parent = ( stacked ? this.nodeDB.get( stacked ) : treeNode.parent() ),
		...
		/**
		 * Create the path which is represented as a point, used for hiding the connection A path with a leading "_" indicates the path will be hidden See: http://dmitrybaranovskiy.github.io/raphael/reference.html#Paper.path
		 * 
		 * @param {object}
		 *            hidePoint
		 * @returns {string}
		...
		getPointPathString : function ( hidePoint ) {
			return [
					"_M", hidePoint.x, ",", hidePoint.y, 'L', hidePoint.x, ",", hidePoint.y, hidePoint.x, ",", hidePoint.y
			].join( ' ' );
		},

		/**
		 * This method relied on receiving a valid Raphael Paper.path. See: http://dmitrybaranovskiy.github.io/raphael/reference.html#Paper.path A pathString is typically in the format of "M10,20L30,40"
		 * 
		 * @param path
		 * @param {string}
		 *            pathString
		...
		getPathString : function ( from_node, to_node, stacked ) {
			...
			if ( stacked ) { // STACKED CHILDREN
				...
				if ( connType == "step" || connType == "straight" ) {
					...
					pathString = [
							"M", sp, 'L', helpPoint, 'S', stackPoint, ep
					];
				}
	...
	TreeNode.prototype.createGeometry = function ( tree ) {
		...
		var drawArea = tree.drawArea, image,
		...
		tree.CONFIG.callback.onCreateNode.apply( tree, [
				this, node
		] );
