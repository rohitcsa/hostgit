...
( function ( jQuery ) {
	...
	jQuery.support.transition = "Moz" + trans in divStyle ? "Moz" + trans : "Webkit" + trans in divStyle ? "Webkit" + trans : false;
	...
	jQuery.fn.extend( {
		/*
		 * show: function( speed, easing, callback ) { var elem, display;
		 * 
		 ...
		 *  // Set elements which have been overridden with display: none // in a stylesheet to whatever the default browser style is // for such an element if ( (display === "" && jQuery.css(elem, "display") === "none") || !jQuery.contains( elem.ownerDocument.documentElement, elem ) ) {
		 ...
		 * 
		 * for ( ; i < j; i++ ) { elem = this[i]; if ( elem.style ) { display = jQuery.css( elem, "display" );
		 * 
		...
		animate : function ( prop, speed, easing, callback ) {
			...
			function doAnimation () {
				...
				supportTransition = !opt.step && jQuery.support.transition, transition, transitions = [], easing, real, lower, computedStyle;
				...
				for ( name in prop ) {
					...
					if ( transition ) {
						...
						transition = lower + " " + opt.duration + "ms " + transition;
				...
				if ( supportTransition && transitions.length ) {
					...
					this.style[ supportTransition ] = transitions.join() + ( transition && transition.indexOf( "none" ) ? "," + transition : "" );
	 ...
	 * 
	 * return obj; }
	 *  // Generate shortcuts for custom animations jQuery.each({ slideDown: genFx( "show", 1 ), slideUp: genFx( "hide", 1 ), slideToggle: genFx( "toggle", 1 ), fadeIn: { opacity: "show" }, fadeOut: { opacity: "hide" }, fadeToggle: { opacity: "toggle" } }, function( name, props ) { jQuery.fn[ name ] =
	 * function( speed, easing, callback ) { return this.animate( props, speed, easing, callback ); }; });
	 * 
	...
	jQuery.extend( jQuery.fx.prototype, {
		...
		custom : function ( from, to, unit ) {
			...
			if ( ( t.transition = transition[ prop ] ) ) {
				...
				transition[ prop ].styleToSet = [
						self.elem, prop, to + self.unit
				];
		...
		/*
		 * // Simple 'show' function show: function() { var dataShow = jQuery._data( this.elem, "fxshow" + this.prop );
		 *  // Remember where we started, so that we can go back to it later this.options.orig[ this.prop ] = dataShow || jQuery.style( this.elem, this.prop ); this.options.show = true;
		 *  // Begin the animation // Make sure that we start at a small width/height to avoid any flash of content if ( dataShow !== undefined ) { // This show is picking up where a previous hide or show left off this.custom( this.cur(), dataShow ); } else { this.custom( this.prop === "width" ||
		 * this.prop === "height" ? 1 : 0, this.cur() ); }
		 *  // Start by showing the element jQuery( this.elem ).show(); },
		 *  // Simple 'hide' function hide: function() { // Remember where we started, so that we can go back to it later this.options.orig[ this.prop ] = jQuery._data( this.elem, "fxshow" + this.prop ) || jQuery.style( this.elem, this.prop ); this.options.hide = true;
		...
		step : function ( gotoEnd ) {
			...
			// ++TRANSITION++
			transition = options.transition[ this.prop ], naturalEnd = t >= options.duration + this.startTime, supportTransition = jQuery.support.transition;
	...
	jQuery.extend( jQuery.fx, {
		tick : function () {
			 ...
			 * 
			 * _default: function( fx ) { if ( fx.elem.style && fx.elem.style[ fx.prop ] != null ) { fx.elem.style[ fx.prop ] = fx.now + fx.unit; } else { fx.elem[ fx.prop ] = fx.now; } } }
			 */
	} );

	/*
	 * // Ensure props that can't be negative don't go there on undershoot easing jQuery.each( fxAttrs.concat.apply( [], fxAttrs ), function( i, prop ) { // Exclude marginTop, marginLeft, marginBottom and marginRight from this list if ( !rMarginProp.test( prop ) ) { jQuery.fx.step[ prop ] =
	 * function( fx ) { jQuery.style( fx.elem, prop, Math.max(0, fx.now) + fx.unit ); }; } });
	 * 
